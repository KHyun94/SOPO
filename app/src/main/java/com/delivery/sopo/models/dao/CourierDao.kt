...
interface CourierDao
{
    ...
    @Query("SELECT courierName, clickRes, nonClickRes, iconRes FROM COURIER WHERE minLen >= :len AND maxLen <= :len ORDER BY priority DESC LIMIT :cnt")
    fun getWithLen(len:Int, cnt : Int) : List<CourierItem>

    @Query("SELECT courierName, clickRes, nonClickRes, iconRes FROM COURIER WHERE minLen >= :len AND maxLen <= :len AND courierName != :param1 ORDER BY priority DESC LIMIT :cnt")
    fun getWithLenAndCondition1(len:Int, param1:String, cnt : Int) : List<CourierItem>

    @Query("SELECT courierName, clickRes, nonClickRes, iconRes FROM COURIER WHERE minLen >= :len AND maxLen <= :len AND courierName != :param1 AND courierName != :param2 ORDER BY priority DESC LIMIT :cnt")

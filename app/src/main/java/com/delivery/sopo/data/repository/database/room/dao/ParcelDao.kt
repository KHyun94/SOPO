...
interface ParcelDao
{
    ...
    @Query("SELECT * FROM PARCEL as p LEFT JOIN PARCEL_STATUS as pm ON p.REG_DT = pm.REG_DT AND p.PARCEL_UID = pm.PARCEL_UID WHERE p.STATUS = 1 AND p.DELIVERY_STATUS = 'delivered' AND pm.isNowVisible = 1")
    ...
    @Query("SELECT * FROM PARCEL as p LEFT JOIN PARCEL_STATUS as pm ON p.REG_DT = pm.REG_DT AND p.PARCEL_UID = pm.PARCEL_UID WHERE p.STATUS = 0 AND pm.isBeDelete = 1")
...
//    @Query("SELECT p.* FROM PARCEL as p INNER JOIN PARCEL_STATUS as pm where p.REG_DT = pm.REG_DT AND p.PARCEL_UID = pm.PARCEL_UID AND p.STATUS = 1  AND p.DELIVERY_STATUS <> 'delivered'")
//    fun getUpdatableInquiryHash(): List<ParcelEntity?>

    @Query("SELECT pm.unidentifiedStatus FROM PARCEL as p INNER JOIN PARCEL_STATUS as pm where p.REG_DT = pm.REG_DT AND p.PARCEL_UID = pm.PARCEL_UID AND p.REG_DT = :regDt AND p.PARCEL_UID = :parcelUid AND p.DELIVERY_STATUS != 'delivered'")
    fun getIsUnidentifiedLiveData(regDt: String, parcelUid: String): LiveData<Int?>

    @Query("SELECT pm.updatableStatus FROM PARCEL as p INNER JOIN PARCEL_STATUS as pm where p.REG_DT = pm.REG_DT AND p.PARCEL_UID = pm.PARCEL_UID AND p.REG_DT = :regDt AND p.PARCEL_UID = :parcelUid AND p.DELIVERY_STATUS != 'delivered'")

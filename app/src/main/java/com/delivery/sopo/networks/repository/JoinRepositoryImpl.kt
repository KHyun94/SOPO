...
class JoinRepositoryImpl: JoinRepository, BaseServiceBeta(), KoinComponent
{
    ...
    override suspend fun requestJoinBySelf(joinInfo: JoinInfo): NetworkResponse<Unit> = withContext(Dispatchers.IO) {
        return@withContext apiCall { joinAPI.requestJoinBySelf(joinInfo) }
    }

    override suspend fun requestJoinByKakao(joinInfo: JoinInfo): NetworkResponse<Unit> = withContext(Dispatchers.IO) {

...
enum class ResponseCode(val CODE: String, val MSG: String, val HTTP_STATUS: Int? = 0) {
    ...
    FIREBASE_ERROR_WEAK_PASSWORD("FB01", "8~20자리의 영문+숫자+특수문자로 비밀번호를 입력해주세요.", 400),
    FIREBASE_ERROR_INVALIDATION_USER("FB02", "올바른 사용자가 아닙니다.", 400),
    FIREBASE_ERROR_INVALID_CREDENTIALS("FB03", "올바른 이메일과 비밀번호를 확인해주세요.", 400),
    FIREBASE_ERROR_TOO_MANY_REQUESTS("FB04", "너무 많은 요청으로 일시적 차단이 되었습니다. 잠시 후 다시 시도해주세요.", 400),
    FIREBASE_ERROR_AUTH_USER_COLLISION("FB05", "이미 가입한 이메일입니다.", 400),
    FIREBASE_ERROR_EMAIL_VERIFIED("FB06", "이메일 인증을 해주세요.", 400),
    FIREBASE_ERROR_AUTH_EMAIL("FB07", "이메일 인증 메일 전송에 실패했습니다. 다시 한번 시도해주세요.", 400),
    FIREBASE_ERROR_NOT_UPDATE_EMAIL("FB08", "이메일 업데이트가 필요합니다..", 400),
    FIREBASE_ERROR_UNKNOWN("FB99", "알 수 없는 에러", 400),
